...
public class ShopViewEditor extends EditorMenu<ExcellentShop, VirtualShop> {
    ...
    private static final TextComponent TYPE_PREFIX = Component.text("Type: ").color(NamedTextColor.AQUA);
    ...
    private @NotNull MenuItemType getType(@NotNull ItemStack item) {
    ...
    private void removeTypeLore(@NotNull ItemStack item) {
        ...
        lore.removeIf(line -> ComponentUtil.asPlainText(line).startsWith(TYPE_PREFIX.content()));
        meta.lore(lore);
    ...
    private void addTypeLore(@NotNull ItemStack item, @NotNull MenuItemType type) {
        ...
        lore.add(0, TYPE_PREFIX.append(Component.text(type.name()).color(NamedTextColor.GREEN)));
        meta.lore(lore);
        item.setItemMeta(meta);
    }
