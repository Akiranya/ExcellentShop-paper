...
public class ProductMainEditor extends EditorMenu<ExcellentShop, VirtualProduct> {
    ...
    public ProductMainEditor(@NotNull ExcellentShop plugin, @NotNull VirtualProduct product) {
        ...
        this.addItem(Material.ENCHANTED_BOOK, VirtualLocales.PRODUCT_RESPECT_PLUGIN_ITEM, 2).setClick((viewer, event) -> {
            if (!(product instanceof ItemProduct itemProduct))
                return;

            itemProduct.setRespectPluginItem(!itemProduct.isRespectPluginItem());
            this.save(viewer);
    ...
    private @NotNull ItemClick getStockClick(@NotNull StockType stockType) {
        return (viewer, event) -> {
            if (event.getClick() == ClickType.DROP) {
