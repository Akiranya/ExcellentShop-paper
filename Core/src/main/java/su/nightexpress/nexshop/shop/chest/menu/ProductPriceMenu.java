    ...
    public void onItemPrepare(@NotNull Player player, @NotNull MenuItem menuItem, @NotNull ItemStack item) {
        ...
        if (menuItem.getType() instanceof Type type) {
            if (type == Type.PRODUCT_CHANGE_PRICE_BUY) {
                ItemUtil.replacePlaceholderListString(item, "%price_buy%", this.formatLorePrice.get(TradeType.BUY).get(this.product.getPricer().getType()));
            } else if (type == Type.PRODUCT_CHANGE_PRICE_SELL) {
                ItemUtil.replacePlaceholderListString(item, "%price_sell%", this.formatLorePrice.get(TradeType.SELL).get(this.product.getPricer().getType()));
            }
        }
        ItemUtil.replaceNameAndLore(item, this.product.replacePlaceholders());
    }
