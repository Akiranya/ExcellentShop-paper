        ...
        this.getItems().forEach(menuItem -> {
            if (menuItem.getType() == Type.PRODUCT_CHANGE_PRICE_REFRESH) {
                ...
                menuItem.getOptions().addDisplayModifier((viewer, item) -> {
                    ItemUtil.replacePlaceholderListString(item, "%price_buy%", this.formatLorePrice.get(TradeType.BUY).get(this.product.getPricer().getType())); // Mewcraft
                });
            } else if (menuItem.getType() == Type.PRODUCT_CHANGE_PRICE_SELL) {
                menuItem.getOptions().addDisplayModifier((viewer, item) -> {
                    ItemUtil.replacePlaceholderListString(item, "%price_sell%", this.formatLorePrice.get(TradeType.SELL).get(this.product.getPricer().getType())); // Mewcraft
                });
            }
            menuItem.getOptions().addDisplayModifier((viewer, item) -> ItemUtil.replaceNameAndLore(item, this.product.replacePlaceholders()));
        });
    }
