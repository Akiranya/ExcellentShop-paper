...
public class ShopBankMenu extends PlayerEditorMenu implements AutoPaged<Currency> {
    ...
    private final Component objectName;
    private final List<Component> objectLore;
    ...
    public ShopBankMenu(@NotNull ChestShop shop) {
        ...
        this.objectName = cfg.getComponent("Currency.Name", Component.empty()); // Mewcraft - no legacy color
        this.objectLore = cfg.getComponentList("Currency.Lore"); // Mewcraft - no legacy color
    ...
    public ItemStack getObjectStack(@NotNull Player player, @NotNull Currency currency) {
        ...
        icon.editMeta(meta -> {
            meta.displayName(this.objectName);
            meta.lore(this.objectLore);
            ItemUtil.replaceNameAndLore(meta,
                currency.replacePlaceholders(),
                str -> str
