...
public class ShopMainEditor extends EditorMenu<ExcellentShop, VirtualShop> {
    ...
    public ShopMainEditor(@NotNull ExcellentShop plugin, @NotNull VirtualShop shop) {
        ...
        this.addItem(ItemUtil.createCustomHead(TEXTURE_BOOK), VirtualLocales.SHOP_DESCRIPTION, 13).setClick((viewer, event) -> {
            ...
            this.startEdit(viewer.getPlayer(), plugin.getMessage(VirtualLang.EDITOR_ENTER_DESCRIPTION), chat -> {
                shop.getDescription().add(chat.getMessage());
                this.save(viewer);
                return true;
            });
        });
        ...
        this.addItem(shop.getIcon(), 14).setClick((viewer, event) -> {
            ...
            item.editMeta(meta -> {
                meta.displayName(ComponentUtil.asComponent(VirtualLocales.SHOP_ICON.getLocalizedName()));
                meta.lore(ComponentUtil.asComponent(VirtualLocales.SHOP_ICON.getLocalizedLore()));
            ...
            if (event.isShiftClick()) {
                if (event.isLeftClick()) {
                    this.startEdit(viewer.getPlayer(), plugin.getMessage(VirtualLang.EDITOR_ENTER_TITLE), chat -> {
                        shop.getView().getOptions().setTitle(chat.getMessage());
                        this.save(viewer);
                        return true;
                    });
                } else {
        ...
        this.getItems().forEach(menuItem -> {
            if (menuItem.getOptions().getDisplayModifier() == null) {
                menuItem.getOptions().setDisplayModifier((viewer, item) -> ItemUtil.replaceNameAndLore(item, shop.replacePlaceholders()));
            }
        });
    }
    ...
    public @NotNull DiscountListEditor getDiscountEditor() {
        if (this.discountEditor == null) {
    ...
    public @NotNull ShopViewEditor getViewEditor() {
        if (this.viewEditor == null) {
    ...
    public @NotNull ProductListEditor getProductsEditor() {
        if (this.productEditor == null) {
