...
public interface IScheduled {
    ...
    static @NotNull Set<LocalTime> parseTimes(@NotNull List<String> list) {
        return list.stream().map(timeRaw -> LocalTime.parse(timeRaw, TIME_FORMATTER)).collect(Collectors.toSet());
    }

    @Deprecated static @NotNull Set<LocalTime> parseTimesOld(@NotNull List<String> list) {
        return list.stream().map(timeRaw -> LocalTime.parse(timeRaw.split("-")[0], TIME_FORMATTER)).collect(Collectors.toSet());
    }

    static @NotNull Set<DayOfWeek> parseDays(@NotNull String str) {
    ...
    default @Nullable BukkitTask createScheduler() {
        ...
        return ShopAPI.PLUGIN.getScheduler().runTaskLater(ShopAPI.PLUGIN, this.getCommand(), delay);
    ...
    private @Nullable LocalDateTime getClosestDate() {
