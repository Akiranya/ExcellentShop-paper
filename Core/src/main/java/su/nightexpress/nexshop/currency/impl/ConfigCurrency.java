...
public class ConfigCurrency extends AbstractConfigHolder<ExcellentShop> implements Currency {
    ...
    public boolean load() {
        ...
        this.name = cfg.getString("Name", StringUtil.capitalizeUnderscored(this.getId())); // Mewcraft - no legacy color
        this.format = cfg.getString("Format", Placeholders.GENERIC_PRICE + " " + Placeholders.CURRENCY_NAME); // Mewcraft - no legacy color
